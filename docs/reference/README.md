...
| [pipeline](pipeline.md) | Forge pipeline — `ForgeState`, `Agents`, runner (`run_project`/`resume_project`), router, decisions, git-style snapshots |
...
| [agent_sdk](agent_sdk.md) | Claude Agent SDK driver — 9 helix tools (Atlas + snapshot reads + gated run/resume) as in-process MCP |
| [docker](docker.md) | Docker sandbox execution — `run_sandboxed` |
| [cli](cli.md) | CLI commands — `run`, `agent`, `snapshots` (list/show/diff/diagram/resume), `setup`, `init`, `status`, `log`, `atlas search` |
